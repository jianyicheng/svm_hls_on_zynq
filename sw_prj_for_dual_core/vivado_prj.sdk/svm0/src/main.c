...
/*
 * main.c: simple test application for Core 0
 *
...
short *classifier_buffer = &testData_int[0];
...
int main()
{
    ...
    print("\r\nCore 0 Build @ " __DATE__ " " __TIME__ "\r\n");
    	// print("Core 0 Program to test communication with HLS MACC block in PL\n\r");
    	...
    	// int data_set_ctr = 0;
    	   ...
    	   if(status != XST_SUCCESS){
    	      print("Core 0 HLS peripheral setup failed\n\r");
    	      exit(-1);
    	   }
    	   //Setup the interrupt
    	   status = setup_interrupt();
    	   if(status != XST_SUCCESS){
    	      print("Core 0 Interrupt setup failed\n\r");
    	      exit(-1);
    	   }
    	   ...
    	   /*
    	   printf("Core 0 Precalculation: \n");
    	   printf("Core 0 time test start: %llu \n", start_time);
    	   printf("Core 0 time test end : %llu \n", end_time);
    	   */
    	   // printf("Core 0 time offset : %d \n", dt);



    	   if (XSvm_classifier_IsReady(&classifier))
    	      print("Core 0 HLS peripheral is ready.  Starting... \n");
    	   else {
    	      print("!!! Core 0 HLS peripheral is not ready! Exiting...\n\r");
    	      exit(-1);
    	   }

    	   if (0) {
    		   	 // print("\n\r---------------Core 0 Interrupt test-------------- \n\r");
    		     ...
    		     printf("Core 0 time difference : %llu \n", t);
    		     printf("Core 0 Actual hardware time: %f ms.\n", ((float)t)*3e-6);
    	         ...
    	         printf("Quick look: Core 0 HW error rate: %f\n\r", ((float)kk/1000.0));

    	   // Floating point SW verification process
    	   float *sw_ip = &testData[0];
    	   ...
    	   XTime_GetTime(&end_time);
    	   // printf("Core 0 Floating point SW: \n");
    	   // printf("Core 0 time test start: %llu \n", start_time);
    	   // printf("Core 0 time test end : %llu \n", end_time);
    	   t = end_time-start_time-dt;
    	   // printf("Core 0 time difference : %llu \n", t);
    	   printf("Core 0 software hardware time: %f s.\n", ((float)t)*3e-9);
    	   ...
    	   if (hw_error == sw_error) {
    	      printf("*** Core 0 Results match: Error rate = %f ***\n\r", ((float)hw_error/10.0));
    	      status = 0;
    	   }
    	   else {
    	      printf("!!! Core 0 MISMATCH !!!\n\r Error rate = %f \n\r", ((float)hw_error/10.0));
    	      status = -1;
    	   }
...
int setup_interrupt()
{
   ...
   if (pCfg == NULL){
      print("Core 0 Interrupt Configuration Lookup Failed\n\r");
      return XST_FAILURE;
   }
